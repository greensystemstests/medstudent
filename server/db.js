...
export async function migrate(db) {
  await db.query(`
    ...
    CREATE TABLE IF NOT EXISTS applications (
      ...
      form jsonb NOT NULL, current_step int NOT NULL DEFAULT 1, version int NOT NULL DEFAULT 1,
      status text NOT NULL DEFAULT 'draft', payment_intent_id text UNIQUE,
      checkout_attempt int NOT NULL DEFAULT 0, checkout_started_at timestamptz,
    ...
    CREATE TABLE IF NOT EXISTS application_consents (
      ...
      application_version int NOT NULL, form_snapshot jsonb NOT NULL, accepted_at timestamptz NOT NULL DEFAULT now(),
      UNIQUE(application_id, policy_version, application_version)
    );
    CREATE TABLE IF NOT EXISTS invoice_counter (id int PRIMARY KEY CHECK(id=1), last_number bigint NOT NULL);
    CREATE TABLE IF NOT EXISTS invoices (
      payment_intent_id text PRIMARY KEY, invoice_number bigint UNIQUE NOT NULL,
      issued_at timestamptz NOT NULL, buyer_email text NOT NULL, payload jsonb NOT NULL, pdf bytea,
      created_at timestamptz NOT NULL DEFAULT now()
    );
    ...
    CREATE TABLE IF NOT EXISTS staff_audit (
      ...
      subject_user_id uuid REFERENCES users(id), action text NOT NULL, detail jsonb NOT NULL,
      created_at timestamptz NOT NULL DEFAULT now()
    );
  `);
}
    ...
    if (rows[0] && Number(rows[0].age) < LOGIN_CODE_RESEND_SECONDS) {
      return {
        retryAfterSeconds: Math.ceil(
          LOGIN_CODE_RESEND_SECONDS - Number(rows[0].age),
        ),
      };
    }
...
export async function verifyLoginCode(db, email, code) {
  return transaction(db, async (db) => {
    ...
    if (!/^\d{6}$/.test(code) || !sameHash(entry.code_hash, sha256(code))) {
      return {
        error:
          "That code is not correct. Please check the email and try again.",
      };
    }
    await db.query(`DELETE FROM login_codes WHERE email = $1`, [email]);
...
export async function setFullNameIfMissing(db, userId, fullName) {
  ...
  await db.query(
    ...
    [userId, fullName.slice(0, 120)],
  );
}

export async function logActivity(db, userId, type, detail = "") {
  await db.query(
    `INSERT INTO activity (user_id, type, detail) VALUES ($1, $2, $3)`,
    [userId, type, detail.slice(0, 300)],
  );
}
...
const DOCUMENT_COLUMNS = `id, category, filename, mime_type, size_bytes, status, review_note, reviewed_at, created_at`;
...
export async function getDocumentWithContent(db, userId, documentId) {
  const { rows } = await db.query(
    `SELECT ${DOCUMENT_COLUMNS}, user_id, key_id, iv, auth_tag, content FROM documents WHERE id = $1 AND user_id = $2`,
    [documentId, userId],
  );
  return rows[0] || null;
}
...
export async function insertDocumentWithinQuota(db, doc, limits) {
  return transaction(db, async (client) => {
    ...
    const usage = await documentUsage(client, doc.userId);
    if (
      usage.count >= limits.count ||
      usage.bytes + doc.sizeBytes > limits.bytes
    ) {
      throw Object.assign(
        new Error(
          "Document storage limit reached. Delete a document before uploading another.",
        ),
        { status: 409 },
      );
    }
