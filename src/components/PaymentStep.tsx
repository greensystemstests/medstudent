...
const appearance: StripeElementsOptions['appearance'] = {
  ...
  rules: {
    ...
    '.Tab--selected': { borderColor: '#006644', boxShadow: '0 0 0 1px #006644' },
  },
};

export const PaymentStep: React.FC<PaymentStepProps> = ({ app, onIntentCreated, onPaid }) => {
  ...
  // Create (or refresh) the PaymentIntent with the applicant's latest details whenever this step opens.
  useEffect(() => {
    let cancelled = false;
    setLoadError(null);
    Promise.all([getPaymentConfig(), createOrUpdatePaymentIntent(app)])
      .then(([cfg, pi]) => {
        if (cancelled) return;
        setConfig(cfg);
        setIntent(pi);
      ...
      .catch((err: unknown) => {
        ...
        setLoadError(
          err instanceof ApiError && err.status === 503
            ? 'Online payments are being set up. Please try again shortly.'
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200" id="payment-step">
      <div className="border-b border-slate-100 pb-4">
        ...
        <h2 className="text-lg font-bold font-heading text-slate-900">Step 8: Your Onboarding Package & Payment</h2>
        <p className="text-xs text-slate-500 mt-1">
          Here's exactly what your one-time {total} fee covers, and what happens on your consultation call.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
        ...
        <div className="xl:col-span-7 space-y-5">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5">
            <div className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-3">What's included</div>
              ...
              {ONBOARDING_INCLUSIONS.map((item) => (
                <li key={item.title} className="flex items-start gap-2.5 text-xs">
                  ...
                  <span className="text-slate-600">
                    <strong className="text-slate-900">{item.title}:</strong> {item.detail}
                  </span>
                </li>
              ))}
            </ul>
            ...
            <div className="rounded-xl bg-white border border-emerald-100 p-3 text-xs flex items-start gap-2">
              ...
              <div>
                <div className="font-bold text-slate-900">
                  Requested: {form.consultationDate ? formatDay(form.consultationDate, { weekday: 'long', day: 'numeric', month: 'long' }) : '—'}
                  {' · '}
                  {form.consultationWindow}
                </div>
                <div className="text-slate-500 mt-0.5 flex items-center gap-1">
                  <Globe2 className="w-3 h-3" />
                  We'll confirm the exact time and send your Zoom link to {form.email}.
                </div>
              </div>
            </div>

            <div>
              <div className="text-xs font-bold text-slate-800 mb-2">What we'll cover on the call</div>
        ...
        <div className="xl:col-span-5 xl:sticky xl:top-28">
          <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
            ...
            <div className="p-5 space-y-4">
              ...
              {loadError ? (
                <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-xs text-red-800 space-y-3" role="alert">
                ...
                <Elements key={elementsOptions.clientSecret} stripe={getStripe(config.publishableKey)} options={elementsOptions}>
...
const CheckoutForm: React.FC<{
  ...
  const verify = async (paymentIntentId: string) => {
    ...
    setProcessing(false);
    setMessage('Your payment has not completed yet. If you were charged, contact us with your email and we will sort it out.');
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4" id="checkout-form">
      <PaymentElement
        ...
        options={{
          layout: 'tabs',
          defaultValues: { billingDetails: { name: app.form.fullName, email: app.form.email } },
        }}
      />

      {message && (
        <div className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-800 flex items-start gap-2" role="alert">
          <AlertCircle className="w-4 h-4 shrink-0 mt-0.5" />
          <span>{message}</span>
        </div>
      )}
