import { OnlineSave } from "./ApplicationWorkspace";
import { preliminaryResult } from "../../shared/admissions.js";
...
const ENGLISH_OPTIONS: {
  value: WizardFormData["englishProficiency"];
  label: string;
}[] = [
...
export const WizardView: React.FC<WizardViewProps> = ({
  ...
  const currentStep =
    app.status === "checkout"
      ? PAYMENT_STEP
      : Math.min(app.currentStep, reachable);
  const errors =
  ...
  const update = (patch: Partial<WizardFormData>) =>
    onChange((a) => ({ ...a, form: { ...a.form, ...patch } }));

  const goTo = (step: number) => {
    if (app.status === "checkout") return;
          ...
          >
            {showErrors && errors.length > 0 && (
              <div
                ...
                role="alert"
                tabIndex={-1}
                className="rounded-xl border border-red-200 bg-red-50 p-4 text-xs text-red-800"
              >
                <div className="font-bold flex items-center gap-1.5 mb-1.5">
                  <AlertCircle className="w-4 h-4" /> Please complete this step
                  before continuing
                </div>
            ...
            {currentStep === 1 && (
              <div className="space-y-6 animate-in fade-in duration-200">
                ...
                <div>
                  ...
                  <div
                    role="group"
                    aria-labelledby="grp-immigration"
                  ...
                  >
                    {(
                      ...
                      <button
                        ...
                        aria-pressed={form.nationalityCategory === value}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        update({ citizenshipCountry: e.target.value })
                      }
                    />
                  </div>
                  <div>
                    <label className={labelClass} htmlFor="school-country">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        update({ schoolCountry: e.target.value })
                      }
                    />
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        update({
                          highSchoolCurriculum: e.target
                            .value as WizardFormData["highSchoolCurriculum"],
                        })
                      }
                    >
                      <option value="tawjihi">
                        Jordanian Tawjihi (Scientific Stream)
                      </option>
                      <option value="ib">
                        International Baccalaureate (IB)
                      </option>
                      <option value="a_levels">British A-Levels</option>
                      <option value="american_diploma">
                        US High School Diploma + AP
                      </option>
                      <option value="national_curriculum">
                        Other National Secondary School
                      </option>
                    </select>
                  </div>
                  <div>
                ...
                <Notice>
                  Your passport country and the country issuing your school
                  qualification may differ. Confirm the university’s document
                  requirements and the applicable legalization route before
                  ordering an apostille, translation or courier service.
                </Notice>
              </div>
            )}
            ...
            {currentStep === 2 && (
              <div className="space-y-6 animate-in fade-in duration-200">
                ...
                <div>
                  ...
                  <div
                    role="group"
                    aria-labelledby="grp-degree"
                    className="grid grid-cols-3 gap-3"
                  >
                    {(
                      ["Medicine", "Dentistry", "Pharmacy"] as DegreeProgram[]
                    ).map((deg) => (
                      <button
                        key={deg}
                        type="button"
                        onClick={() => {
                          ...
                          update({
                            degree: deg,
                            universityId:
                              uni && !uni.programs.includes(deg)
                                ? ""
                                : form.universityId,
                          });
                        }}
                        aria-pressed={form.degree === deg}
                ...
                <div>
                  ...
                  <div
                    role="group"
                    aria-labelledby="grp-university"
                    ...
                    {UNIVERSITIES.map((uni) => {
                      ...
                      return (
                        <button
                          ...
                          aria-pressed={form.universityId === uni.id}
                        ...
                        >
                          <div className="flex items-center justify-between mb-1">
                            ...
                            <span className="text-[0.6875rem] font-bold text-[#006644] bg-emerald-100/70 px-2 py-0.5 rounded">
                              {uni.shortName}
                            </span>
                          </div>
                ...
                <div>
                  ...
                  >
                    {INTAKE_OPTIONS.map((opt) => (
                      <button
                        key={opt.value}
                        type="button"
                        onClick={() => update({ intakeSeason: opt.value })}
                        aria-pressed={form.intakeSeason === opt.value}
                        className={`p-3.5 ${choiceClass(form.intakeSeason === opt.value)}`}
                      >
                  ...
                  <p className="text-[0.6875rem] text-slate-500 mt-2">
                    Your advisor confirms the exact intake year and deadlines on
                    your call.
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-6 animate-in fade-in duration-200">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  {(
                    ...
                    const low = grade != null && (grade < 0 || grade > 100);
                    return (
                      <div
                        key={key}
                      ...
                      >
                        ...
                        <input
                          ...
                          className={`w-full bg-white border rounded-lg px-3 py-2 text-base font-bold outline-none ${
                            low
                              ...
                              : "border-slate-300 text-[#006644] focus:border-[#006644]"
                          }`}
                        />
                        <p
                          id={`${key}-hint`}
                        ...
                        >
                          {low ? `Enter a percentage from 0 to 100. ` : ""}
                          Convert to a percentage if your system uses points or
                          letters.
                        </p>
                      </div>
                    );
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      update({
                        englishProficiency: e.target
                          .value as WizardFormData["englishProficiency"],
                      })
                    }
                  >
            ...
            {currentStep === 5 && (
              <div className="space-y-6 animate-in fade-in duration-200">
                ...
                <div
                  role="group"
                  aria-label="Entrance exam session"
                  className="grid grid-cols-1 sm:grid-cols-2 gap-3"
                >
                  {examSessions().map((session) => {
                    ...
                    return (
                      <button
                        ...
                        aria-pressed={form.examDate === session.date}
                        className={`p-3.5 disabled:opacity-50 disabled:cursor-not-allowed ${choiceClass(form.examDate === session.date)}`}
                      >
                        <div className="font-bold text-xs text-slate-900 flex items-center justify-between">
                          {session.label}
                          {!open && (
                            <span className="text-[0.6875rem] font-bold text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded">
                              Closed
                            </span>
                          )}
                        </div>
                        <div className="text-[0.6875rem] text-slate-500">
                          {open ? session.note : "Registration has closed"}
                        </div>
                      </button>
                    );
                  })}
                  <button
                    type="button"
                    onClick={() =>
                      update({ examDate: EXAM_DECIDE_WITH_ADVISOR })
                    }
                    aria-pressed={form.examDate === EXAM_DECIDE_WITH_ADVISOR}
              ...
              <div className="space-y-6 animate-in fade-in duration-200">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                  <ReviewCard title="Applicant" onEdit={() => goTo(1)}>
                    ...
                    <div>
                      {form.citizenshipCountry} · graduated{" "}
                      {form.graduationYear}
                    </div>
                  </ReviewCard>
                  ...
                  <ReviewCard title="Academics" onEdit={() => goTo(3)}>
                    <div>
                      Biology {form.biologyGrade}% · Chemistry{" "}
                      {form.chemistryGrade}%
                    </div>
                    <div>
                      {
                        ENGLISH_OPTIONS.find(
                          (o) => o.value === form.englishProficiency,
                        )?.label
                      }
                    </div>
                  </ReviewCard>
                ...
                <div className="rounded-2xl border border-emerald-200 bg-emerald-50/40 p-5 space-y-4">
                  <div className="flex items-center gap-3">
                    <div>
                      <div className="font-bold text-sm text-slate-900 flex items-center gap-1.5">
                        <CalendarClock className="w-4 h-4 text-[#006644]" />{" "}
                        Request your 45-min call with StudyBg admissions team
                      </div>
                      <div className="text-[0.6875rem] text-slate-500">
                        Consultation request included in your €180 fee.
                      </div>
                    </div>
                  </div>
                  <div>
                    ...
                    >
                      {callDays.map((day) => (
                        <button
                          ...
                          aria-pressed={form.consultationDate === day}
                  ...
                  <div>
                    ...
                    >
                      {CONSULTATION_WINDOWS.map((w) => (
                        <button
                          key={w}
                          type="button"
                          onClick={() => update({ consultationWindow: w })}
                          aria-pressed={form.consultationWindow === w}
                          className={`px-3 py-2 text-xs font-semibold ${choiceClass(form.consultationWindow === w)}`}
                        >
                          {w}
                        </button>
                      ))}
                    ...
                    <p className="text-[0.6875rem] text-slate-500 mt-2 flex items-center gap-1">
                      <Globe2 className="w-3 h-3" /> Your timezone: {timezone}.
                ...
                <div className="space-y-2 text-xs">
                  {(
                    [
                      [
                        "accuracySigned",
                        "I confirm that my secondary credentials and grades are authentic and accurate.",
                      ],
                      [
                        "termsAgreed",
                        <>
            ...
            <div className="pt-6 border-t border-slate-100 flex items-center justify-between">
              <button
                ...
                disabled={currentStep === 1 || app.status === "checkout"}
              ...
              <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-xs space-y-4">
                ...
                <div className="border-b border-slate-100 pb-3">
                  <div className="font-bold text-sm text-slate-900">
                    {selectedUni?.shortName ?? "University not selected"}
                  </div>
...
const StepHeading: React.FC<{ title: string; subtitle: string }> = ({
  title,
  subtitle,
}) => (
  ...
  <div className="rounded-xl border border-slate-200 bg-slate-50 p-3.5 space-y-0.5 text-slate-600 min-w-0">
    <div className="flex items-center justify-between mb-1">
      ...
      <button
        type="button"
        onClick={onEdit}
        className="text-[0.6875rem] font-semibold text-[#006644] hover:underline inline-flex items-center gap-1"
      >
